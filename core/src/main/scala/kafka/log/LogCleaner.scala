...
class LogCleaner(val config: CleanerConfig,
   ...
   */
  // 执行日志压缩的线程，真正的日志压缩逻辑在其中封装的Cleaner对象中
  private class CleanerThread(threadId: Int)
    ...
    // 参数介绍：
    //  SkimpyOffsetMap:dirty部分的消息建立key与last_offset的映射关系，底层使用ByteBuffer，冲突解决方式为线性探查法
    // ioBufferSize：指定了读写LogSegment的ByteBuffer大小
    // maxBufferSize：指定消息的最大长度
    // dupBufferLoadFactor：指定了SkimpyOffsetMap的最大占用比例
    ...
    private def cleanOrSleep() {
      ...
      cleanerManager.grabFilthiestLog() match {
        case None =>
          ...
          // 没有需要压缩的，CleanerThread线程会睡眠一段时间后，重新执行doWork方法
...
private[log] class Cleaner(val id: Int,
   ...
   * @return The first offset not cleaned*
    * 首先，确定日志压缩最大offset上限upperBoundOffset
    * 从firstDirtyOffset开始遍历LogSegment，并填充OffsetMap，在OffsetMap中记录每个key应该保留的消息offset
    *   当offsetMap被填充满时，就可以确定日志压缩的实际上限endOffset
    * 根据deleteRetentionMS配置，计算可以安全删除的"删除标识"
    * 将logStartOffset到endOffset之间的LogSegment进行分组，并按照分组进行日志压缩
   */
  private[log] def clean(cleanable: LogToClean): Long = {
    ...
    // 步骤一：应该activeSegment不参与日志压缩，所以可以确定日志压缩的最大上限是activeSegment.baseOffset
    val upperBoundOffset = log.activeSegment.baseOffset
    // 步骤二：填充OffsetMap,确定日志压缩的真正上限
    ...
    // 步骤三：计算可以安全删除“删除标识”（即value为空的消息）的LogSegment
    val deleteHorizonMs =
    ...
    // 步骤四：对要压缩的Segment进行分组，并按照分组进行clean
  ...
  private[log] def cleanSegments(log: Log,
    ...
    // 创建.clean后缀的日志文件和索引文件，文件名是分组中第一个LogSegment的baseOffset
    val logFile = new File(segments.head.log.file.getPath + Log.CleanedFileSuffix)
    logFile.delete()
    val indexFile = new File(segments.head.index.file.getPath + Log.CleanedFileSuffix)
    indexFile.delete()

    // 创建对应的FileMessageSet对象，offsetIndex对象以及LogSegment对象
    ...
    try {
      ...
      // 该方法，首先将.clean修改为.swap
      // 将cleaned对象加入到segments跳表中管理
      // 将分组中的LogSegment从segments中删除
      // 最后将.swap后缀名删除
    ...
    while (position < source.log.sizeInBytes) {
      ...
      for (entry <- messages.shallowIterator) {
        ...
        if (entry.message.compressionCodec == NoCompressionCodec) {
          // 判断是否保留这个消息
          // 条件：此消息是否含有key
          // 条件：OffsetMap是否有相同key，且offset更大的消息
          // 条件：此消息是“删除标识”且此LogSegment中的“删除标识”可以安全删除
          if (shouldRetainMessage(source, map, retainDeletes, entry)) {
            // 将需要保留的消息写入writeBuffer
      ...
      // 如果有需要保留的消息，则将其追加到压缩后的LogSegment中
      ...
      // 未读取到一个完整的消息，表示readBuffer过小，需要扩容
   ...
   */
  // 从firstDirtyOffset开始遍历LogSegment
  // offsetMap填充满后，就可以确定endOffset的位置
  private[log] def buildOffsetMap(log: Log, start: Long, end: Long, map: OffsetMap): Long = {
    map.clear()
    // 从start到end的所有LogSegment
    ...
    for (segment <- dirty if !full) {
      ...
      if (newOffset > -1L)
        ...
        // 检查OffsetMap是否已经填满了，如果offset>start，则表示OffsetMap中记录最后一个LogSegment的一部分消息
  ...
  private def buildOffsetMapForSegment(topicAndPartition: TopicAndPartition, segment: LogSegment, map: OffsetMap): Long = {
    ...
    while (position < segment.log.sizeInBytes) {
      ...
      // 遍历message，遍历ByteBufferMessageSet(深层迭代)
      for (entry <- messages) {
      ...
      // position没有移动，表示没有读取到一个完整的Message，对readBuffer和writeBuffer进行扩容，之后重新读取
