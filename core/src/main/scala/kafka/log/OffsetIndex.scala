 ...
 */
/**
  * 用来管理磁盘上的一个索引文件，与FileMessageSet共同构成了LogSegment对象
  * 每个索引项8个字节，前四个字节：相对offset，后四个字节：物理地址
  * 使用MappedByteBuffer将索引文件映射到内存中
...
class OffsetIndex(@volatile private[this] var _file: File, val baseOffset: Long, val maxIndexSize: Int = -1) extends Logging {
   ...
   */
  // 查找目标是小于targetOffset的最大offset对应的物理地址（position）
  def lookup(targetOffset: Long): OffsetPosition = {
    ...
    maybeLock(lock) {
      ...
      if(slot == -1)
        ...
        // 将offset和物理地址（position）封装成OffsetPosition对象
  ...
  private def indexSlotFor(idx: ByteBuffer, targetOffset: Long): Int = {
    ...
    // 如果找不到，返回小于targetOffset的最大索引项位置
    lo
  }
