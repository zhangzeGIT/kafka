...
class FileMessageSet private[kafka](@volatile var file: File,
  ...
  // 将position移动到最后一个字节，之后就可以从此position开始写消息
   ...
   */
  // 查找指定消息的功能
  // 从指定的startingPosition开始逐条遍历FileMessageSet中的消息
  // 并将每个消息的offset与targetOffset进行比较，直到offset大于等于targetOffset，最后返回查找到的offset
  // 只读取LogOverhead（offset和size），并通过size定位下一条消息的开始位置
  def searchFor(targetOffset: Long, startingPosition: Int): OffsetPosition = {
    // 起始位置
    var position = startingPosition
    // 创建用于读取LogOverhead（即offset和size）的byteBuffer长度为十二
    val buffer = ByteBuffer.allocate(MessageSet.LogOverhead)
    // 当前FileMessageSet的大小，单位是字节
    val size = sizeInBytes()
    // 从position开始逐条遍历
    while(position + MessageSet.LogOverhead < size) {
      // 重置ByteBuffer的position指针，准备读入数据
      buffer.rewind()
      // 读取LogOverhead，这里会确保startPosition位于一个消息的开头
      channel.read(buffer, position)
      // 未读取到12个字节的LogOverhead，抛异常
      ...
      // 这里将offset和对应的position封装成OffsetPosition这种对象后返回
    ...
    // 找不到offset大于等于targetOffset的消息，返回空
    null
  }
  ...
  def writeTo(destChannel: GatheringByteChannel, writePosition: Long, size: Int): Int = {
    ...
    val bytesTransferred = (destChannel match {
      // 底层调用的是fileChannel.transferTo方法，实现零拷贝
