...
object ByteBufferMessageSet {
  ...
  // 将message集合按照指定的压缩类型进行压缩，用于构建此对象
  private def create(offsetAssigner: OffsetAssigner,
    ...
    // 情况一：处理message集合为空的情况，返回空ByteBuffer
    if (messages.isEmpty)
      MessageSet.Empty.buffer
    // 情况二：不需要对message集合进行压缩
    // 通过OffsetAssigner分配每个消息的offset（功能是存储一串offset值，并像迭代器那样逐个返回）
    // 在将消息写入ByteBuffer之后，返回ByteBuffer
      ...
      buffer
    // 情况三：处理压缩情况
    // 现将message集合按照指定的压缩方式进行压缩并保存到缓冲区
    // 同时也完成offset的分配
    // 然后按照压缩消息的格式写入外层消息，最后将整个外层消息所在的ByteBuffer返回
    } else {
      // 得到Magic值和时间戳
      ...
      messageWriter.write(codec = compressionCodec, timestamp = magicAndTimestamp.timestamp, timestampType = timestampType, magicValue = magicAndTimestamp.magic) { outputStream =>
        ...
        try {
          ...
          for (message <- messages) {
            ...
            // Magic值为1，写入相对offset，为0，写入的是offset
...
class ByteBufferMessageSet(val buffer: ByteBuffer) extends MessageSet with Logging {
  ...
  private[kafka] def validateMessagesAndAssignOffsets(offsetCounter: LongRef,
    ...
    if (sourceCodec == NoCompressionCodec && targetCodec == NoCompressionCodec) {
      ...
      // 检测所有的message的magic value是否与指定的magic value一致
      if (!isMagicValueInAllWrapperMessages(messageFormatVersion)) {
        // Message format conversion
        // 如果存在message的magic value不一致，则需要进行统一，可能导致消息总长度变化
        // 需要创建新的ByteBufferMessageSet，同时进行offset的分配，验证并更新CRC32，时间戳等信息
        ...
        // 处理非压缩消息且magic值统一的情况，由于magic值确定，长度不会改变
        // 主要进行offset分配，验证并更新CRC32，时间戳等信息
      ...
      /**
        ...
        * 下列情况不能复用
        * 情况一：消息压缩类型与broker压缩类型不一致，需要从新压缩
        * 情况二：magic为0，需要重写消息的offset为绝对offset
        * 情况三：magic大于0，但内部压缩消息某些字段需要修改，例如时间戳
        * 情况四：消息格式需要转换
        */
      var inPlaceAssignment =
        sourceCodec == targetCodec && // 检测情况一
          messageFormatVersion > Message.MagicValue_V0// 检测情况二
      ...
      if (!inPlaceAssignment) {
        ...
        // 创建新ByteBufferMessage对象，重新压缩，此时调用上面的create方法进行压缩
