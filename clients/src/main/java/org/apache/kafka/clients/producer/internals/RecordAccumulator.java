...
public final class RecordAccumulator {
    ...
    // 指定每个RecodeBatch底层byte buffer的大小
    ...
    // TopicPartition 和 RecordBatch对应关系
    private final ConcurrentMap<TopicPartition, Deque<RecordBatch>> batches;
    // 未发送完成的record batch集合，底层通过set<RecordBatch>集合实现
    private final IncompleteRecordBatches incomplete;
    // The following variables are only accessed by the sender thread, so we don't need to protect them.
    private final Set<TopicPartition> muted;
    // 使用drain方法批量导出RecordBatch时，为了防止饥饿，使用drainIndex记录上次发送停止时的位置
     ...
     */
    // KafkaProducer.send方法最终会调用此方法将消息追加到RecordAccumulator中
    public RecordAppendResult append(TopicPartition tp,
        ...
        // 统计向RecordsAccumulator追加数据的线程数
        appendsInProgress.incrementAndGet();
        try {
            // check if we have an in-progress batch

            // 步骤一：查找TopicPartiton对应的Deque
            Deque<RecordBatch> dq = getOrCreateDeque(tp);
            synchronized (dq) {
                if (closed)
                    throw new IllegalStateException("Cannot send after the producer is closed.");
                // 步骤二：向Deque最后一个RecordBatch追加Record
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        ...
        // 根据metadata是否又找不到leader副本的 分区
        boolean unknownLeadersExist = false;
        // 是否有线程在阻塞等待buffer pool释放空间
    ...
    public Map<Integer, List<RecordBatch>> drain(Cluster cluster,
        ...
        for (Node node : nodes) {
            ...
            // drainIndex是batches的角标记录上次发送停止时的位置，下次继续从此位置开始发送
            // 若一直从索引0的队列开始发送，可能会出现一直只发送前几个分区的消息的情况，造成分区饥饿
            ...
            do {
                ...
                if (!muted.contains(tp)) {
                    ...
                    if (deque != null) {
                        synchronized (deque) {
                            // 获取队列中第一个record batch
                            RecordBatch first = deque.peekFirst();
                            if (first != null) {
                                boolean backoff = first.attempts > 0 && first.lastAttemptMs + retryBackoffMs > now;
                                // Only drain the batch if it is not during backoff period.
                                if (!backoff) {
                                    // 数量已满，结束循环，一般是一个请求的大小
                                    if (size + first.records.sizeInBytes() > maxSize && !ready.isEmpty()) {
                                        ...
                                        // 从队列中获取一个record batch，并将这个record batch放到ready集合中
                                        // 每个topicpartition只取一个record batch，防止饥饿
